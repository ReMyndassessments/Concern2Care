...
Submitted: ${new Date().toLocaleString()}
  ...
  app.post('/api/classroom/check-teacher-pin-status', async (req, res) => {
    try {
      const validationSchema = z.object({
        teacherEmail: z.string().email()
      });
  ...
  app.post('/api/classroom/reset-pin', async (req, res) => {
    try {
      const validationSchema = z.object({
        teacherEmail: z.string().email(),
  ...
  app.post('/api/classroom/get-security-question', async (req, res) => {
    try {
      const validationSchema = z.object({
        teacherEmail: z.string().email()
      });
